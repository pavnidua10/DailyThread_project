...
const ACHIEVEMENTS = [
  ...
  { id: 'trusted',         icon: '✦',   label: 'Trusted',        desc: 'Reached Verified Voice tier',      check: (d) => d.total >= 70               },
...
/* ─────────────────────────────────────────────────────────────────
   CREDIBILITY PANEL (full, for own profile)
───────────────────────────────────────────────────────────────── */
  ...
  return (
    <div className="cred-panel fade-up">
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:24, flexWrap:'wrap', gap:12 }}>
        <div>
          <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700, margin:'0 0 4px', color:'var(--ink)' }}>Your Credibility Score</h3>
          <p style={{ fontSize:13, color:'var(--muted)', margin:0 }}>How much the community trusts your contributions</p>
        </div>
        <div className="cred-tier-badge" style={{ background:tier.bg, color:tier.color }}>
      ...
      <div style={{ display:'flex', gap:32, alignItems:'flex-start', flexWrap:'wrap', marginBottom:28 }}>
        <div className="cred-arc-wrap">
          <ScoreArc score={data.total} color={tier.color} size={110} />
          <span style={{ fontSize:11, color:'var(--muted)', marginTop:4 }}>out of 100</span>
          {pointsToNext > 0 && (
            <span style={{ fontSize:11, color:tier.color, fontWeight:600, marginTop:6, background:tier.bg, padding:'2px 10px', borderRadius:999 }}>
              +{pointsToNext} pts → {nextTier.label}
            </span>
          )}
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px 28px', flex:1, paddingTop:4 }}>
          {[
            { label:'Articles published', value:data.articlesPublished, color:'var(--ink)' },
            { label:'Peer endorsements',  value:data.endorsedBy,        color:'var(--ink)' },
            { label:'Flags on record',    value:data.flaggedCount,       color: data.flaggedCount > 0 ? '#DC2626' : 'var(--ink)' },
            { label:'Days as member',     value:data.joinedDaysAgo,      color:'var(--ink)' },
      ...
      <div style={{ borderTop:'1px solid var(--border)', paddingTop:20 }}>
        <p style={{ fontSize:11, fontWeight:600, color:'var(--muted)', letterSpacing:'.08em', textTransform:'uppercase', marginBottom:14 }}>Score breakdown — click for details</p>
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          {BREAKDOWN_META.map(({ key, label, max, icon, desc }) => {
            const val = data.breakdown?.[key] ?? 0;
            const pct = Math.round((val / max) * 100);
            const isActive = activeBreakdown === key;
            return (
              <div key={key} style={{ cursor:'pointer' }} onClick={() => setActiveBreakdown(isActive ? null : key)}>
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:5 }}>
                  <span style={{ fontSize:15 }}>{icon}</span>
                  <span style={{ fontSize:13, color:'var(--ink)', flex:1 }}>{label}</span>
                  <span style={{ fontSize:13, fontWeight:700, color:tier.color }}>{val}<span style={{ color:'var(--border)', fontWeight:400 }}>/{max}</span></span>
                  <span style={{ fontSize:11, color:pct >= 80 ? '#059669' : pct >= 50 ? tier.color : 'var(--muted)', fontWeight:600 }}>{pct}%</span>
                </div>
                <div className="cred-bar-track">
                  <div className="cred-bar-fill" style={{ width:`${pct}%`, background: pct >= 80 ? '#059669' : tier.color }} />
                </div>
                {isActive && (
                  <div style={{ fontSize:12, color:'var(--muted)', marginTop:5, padding:'6px 10px', background:'var(--cream)', borderRadius:8, fontStyle:'italic' }}>
                    {desc}
                  </div>
                )}
              </div>
      ...
      {/* How to improve */}
      <div style={{ marginTop:20, padding:'14px 16px', background:'var(--cream)', borderRadius:12, border:'1px solid var(--border)' }}>
        <p style={{ fontSize:12, fontWeight:600, color:'var(--muted)', letterSpacing:'.06em', textTransform:'uppercase', marginBottom:8 }}>How to improve your score</p>
        <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
          {data.breakdown?.articleQuality < 25 && <p style={{ fontSize:12, color:'var(--ink)', margin:0 }}>✍️ Write more articles — quality ratings boost your score up to 30 pts</p>}
          {data.breakdown?.sourceAccuracy < 20 && <p style={{ fontSize:12, color:'var(--ink)', margin:0 }}>📎 Always attach a verified source URL when posting claims</p>}
          {data.breakdown?.debateScore < 8 && <p style={{ fontSize:12, color:'var(--ink)', margin:0 }}>⚖️ Participate in Debate Mode — upvoted arguments earn up to 10 pts</p>}
          {data.flaggedCount > 0 && <p style={{ fontSize:12, color:'#DC2626', margin:0 }}>⚑ You have {data.flaggedCount} flag(s) — these reduce your consistency score</p>}
          {data.total >= 85 && <p style={{ fontSize:12, color:'#059669', margin:0 }}>◆ You've reached the top tier — keep contributing to maintain your Authority status</p>}
        </div>
      </div>

      <p style={{ fontSize:10, color:'var(--border)', textAlign:'right', marginTop:12 }}>Recalculated daily · Last updated today</p>
    </div>
  );
}
...
function AchievementsPanel({ credData }) {
  ...
  return (
    <div className="cred-panel fade-up" style={{ marginBottom:24 }}>
      <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700, margin:'0 0 4px', color:'var(--ink)' }}>Achievements</h3>
      <p style={{ fontSize:13, color:'var(--muted)', margin:'0 0 20px' }}>Badges you've earned through your contributions</p>
      <div style={{ display:'flex', flexWrap:'wrap', gap:10 }}>
        {ACHIEVEMENTS.map(({ id, icon, label, desc, check }) => {
          const unlocked = check(credData);
          return (
            <div key={id} className={`achievement-badge ${unlocked ? '' : 'locked'}`} title={desc}>
              <span style={{ fontSize:22 }}>{icon}</span>
              <span style={{ fontSize:10, fontWeight:600, color:'var(--ink)', textAlign:'center', lineHeight:1.3 }}>{label}</span>
              {unlocked && <span style={{ fontSize:9, color:'#059669', fontWeight:700 }}>UNLOCKED</span>}
            </div>
          );
        })}
      </div>
...
───────────────────────────────────────────────────────────────── */
// Backend: GET /users/:id/activity → array of { type, title, date, meta }
// types: 'article_published' | 'debate_argument' | 'verdict_vote' | 'community_joined' | 'endorsement_received'
...
const ProfilePage = () => {
  ...
  const fetchMyProfile = useCallback(async () => {
    try {
      ...
      setUserId(u._id);
      setProfile({ name:u.name||'', bio:u.bio||'', email:u.email||'', profilePhoto:u.profilePhoto||'', followers:u.followers||[], following:u.following||[] });
      setArticles(articlesRes.data || []);
      setSavedArticles(savedRes.data || []);

      // Pre-fetch credibility for achievements panel
      if (u._id) {
  ...
  const handleSubmit  = async (e) => {
    ...
    try {
      ...
      setMessage('Profile updated!'); setIsEditing(false); fetchMyProfile();
    } catch { setMessage('Failed to update profile.'); }
  };
  const handleSaveToggle = async () => {
    try { const r = await API.get('/articles/articles/saved'); setSavedArticles(r.data); } catch {}
  };
  const handlePhotoChange = async (e) => {
    if (!e.target.files?.[0]) return;
    setUploading(true);
    try {
      const fd = new FormData(); fd.append('file', e.target.files[0]); fd.append('upload_preset','your_upload_preset_here');
      const res = await fetch(`https://api.cloudinary.com/v1_1/dachmhgyt/image/upload`, { method:'POST', body:fd });
      const result = await res.json();
      if (result.secure_url) { await API.put('/profiles/profile', { profilePhoto:result.secure_url }); setProfile(p => ({ ...p, profilePhoto:result.secure_url })); }
    } catch {} finally { setUploading(false); }
  };
  const handleDeletePhoto = async () => {
    try { await API.put('/profiles/profile', { profilePhoto:'' }); setProfile(p => ({ ...p, profilePhoto:'' })); } catch {}
  };
            ...
            <div style={{ display:'flex', gap:32, alignItems:'flex-start', flexWrap:'wrap' }}>
              ...
              <div className="avatar-wrap" style={{ marginTop:-56 }}>
                ...
                <div className="avatar-inner">
                  {profile.profilePhoto ? <img src={profile.profilePhoto} alt="Profile" /> : <span className="avatar-letter">{profile.name?.[0]?.toUpperCase()||'U'}</span>}
                </div>
                <div className="avatar-actions">
                  ...
                  {profile.profilePhoto && <button className="avatar-btn" onClick={handleDeletePhoto} title="Remove photo"><FaTrash size={11} color="#c8553d" /></button>}
                </div>
              ...
              <div style={{ flex:1, minWidth:220 }}>
                ...
                {isEditing ? (
                  <form onSubmit={handleSubmit} style={{ marginTop:20, display:'flex', flexDirection:'column', gap:16 }}>
                    <div className="edit-field"><label>Name</label><input type="text" name="name" value={profile.name} onChange={handleChange} /></div>
                    <div className="edit-field"><label>Bio</label><textarea name="bio" value={profile.bio} onChange={handleChange} rows={3} /></div>
