...
const ACHIEVEMENTS = [
  ...
  { id: 'five_articles', icon: '📚', label: '5 Articles', desc: 'Published 5 articles', check: (d) => (d?.articlesPublished || 0) >= 5 },
  { id: 'trusted', icon: '✦', label: 'Trusted', desc: 'Reached 70+ credibility', check: (d) => (d?.total || 0) >= 70 },
  { id: 'sourced', icon: '📎', label: 'Source Champ', desc: 'Full source accuracy score', check: (d) => (d?.breakdown?.sourceAccuracy || 0) >= 23 },
  { id: 'consistent', icon: '🔥', label: 'Consistent', desc: 'Full consistency score', check: (d) => (d?.breakdown?.consistencyBonus || 0) >= 13 },
  { id: 'debater', icon: '⚖️', label: 'Debater', desc: 'Earned debate score points', check: (d) => (d?.breakdown?.debateScore || 0) >= 5 },
...
function CredibilityPanel({ credData, loading }) {
  ...
  if (!credData) {
    return (
      <div className="cred-panel fade-up">
        <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700, margin:'0 0 6px', color:'var(--ink)' }}>
          Credibility
        </h3>
  ...
  const tier = getTier(credData.total || 0);
  const currentIndex = TIERS.findIndex(t => (credData.total || 0) >= t.min);
  const nextTier = currentIndex > 0 ? TIERS[currentIndex - 1] : null;
  const pointsToNext = nextTier ? Math.max(nextTier.min - (credData.total || 0), 0) : 0;
    ...
    <div className="cred-panel fade-up">
      <div style={{ display:'flex', justifyContent:'space-between', gap:24, flexWrap:'wrap', alignItems:'center' }}>
        <div style={{ flex:'1 1 340px' }}>
          ...
          <div style={{ marginTop:16 }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'var(--muted)', marginBottom:8 }}>
              <span>Progress</span>
              <span>{nextTier ? `${pointsToNext} points to ${nextTier.label}` : 'Top tier reached'}</span>
            </div>
            <div className="cred-bar-track">
              <div
                className="cred-bar-fill"
                style={{ width:`${Math.min(credData.total || 0, 100)}%`, background:tier.color }}
              />
            </div>
          </div>
        </div>
        ...
        <div className="cred-arc-wrap">
          <ScoreArc score={credData.total || 0} color={tier.color} size={130} />
        </div>
      </div>

      <div style={{ marginTop:28 }}>
        <h4 style={{ margin:'0 0 14px', fontSize:15, fontWeight:700, color:'var(--ink)' }}>Score breakdown</h4>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(180px, 1fr))', gap:14 }}>
          {BREAKDOWN_META.map((item) => {
            const value = credData?.breakdown?.[item.key] || 0;
            const isActive = activeBreakdown === item.key;
            return (
              <button
                key={item.key}
                type="button"
                onClick={() => setActiveBreakdown(isActive ? null : item.key)}
                style={{
                  textAlign:'left',
                  border:'1px solid var(--border)',
                  background:isActive ? '#fff7f4' : 'var(--paper)',
                  borderRadius:16,
              ...
              >
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:10 }}>
                  ...
                  <span style={{ fontSize:18, fontWeight:800, color:'var(--accent)' }}>{value}/{item.max}</span>
                </div>
                <p style={{ margin:'10px 0 0', fontSize:13, fontWeight:700, color:'var(--ink)' }}>{item.label}</p>
...
function AchievementsPanel({ credData }) {
  ...
  return (
    <div className="cred-panel fade-up" style={{ marginBottom:24 }}>
      <h3 style={{ fontFamily:"'Playfair Display',serif", fontSize:20, fontWeight:700, margin:'0 0 4px', color:'var(--ink)' }}>
        Achievements
      </h3>
      <p style={{ fontSize:13, color:'var(--muted)', margin:'0 0 20px' }}>
        Milestones unlocked through publishing, consistency, trust, and debate quality.
      </p>

      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))', gap:14 }}>
        {ACHIEVEMENTS.map((badge) => {
          const unlocked = badge.check(credData);
          return (
            <div
              key={badge.id}
            ...
            >
              ...
              <div style={{ fontSize:12, fontWeight:700, color:'var(--ink)', textAlign:'center' }}>
                {badge.label}
              </div>
              <div style={{ fontSize:10, color:'var(--muted)', textAlign:'center', lineHeight:1.4 }}>
                {badge.desc || 'Achievement badge'}
              </div>
            </div>
          );
        })}
    ...
    <>
      ...
      <div className="profile-root">
        ...
        <div className="profile-outer">
          ...
          {activeTab === 'credibility' && <CredibilityPanel credData={credData} loading={credLoading} />}
